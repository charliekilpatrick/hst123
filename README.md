...
**WFC3 masking (`wfc3mask`):** The Python port and the C binary both need **distortion / pixel-area maps** **`UVIS1wfc3_map.fits`**, **`UVIS2wfc3_map.fits`**, and **`ir_wfc3_map.fits`** under **`.../dolphot3.1/wfc3/data/`**. They ship in **`dolphot3.1.WFC3.tar.gz`** (included in a normal **`hst123-install-dolphot`** HST install). If those files are missing, empty, or unreadable (e.g. unsynced cloud storage), **`--run-dolphot`** fails at the first WFC3 image with **`Error reading ... UVIS2wfc3_map.fits`**. Re-extract the WFC3 module or run **`hst123-install-dolphot --force-download`**.
...
- **Drizzle:** `--drizzle-all`, `--drizzle-dim`, `--drizzle-scale`, `--sky-sub`, `--redrizzle`, `--fix-zpt`, `--no-rotation`, `--max-cores` (AstroDrizzle workers; default min(8, CPU count); use `--max-cores 1` to force single-worker)
